...
export const Sidebar: React.FC<SidebarProps> = ({ currentPage, onNavigate }) => {
  ...
  return (
    <aside className="w-64 glass-panel border-r border-slate-800 flex flex-col h-screen sticky top-0 select-none">
      
      {/* Brand Header */}
      <div className="h-16 flex items-center px-6 border-b border-slate-800 space-x-3">
        <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-brand-600 via-indigo-600 to-emerald-500 flex items-center justify-center shadow-glow-brand">
          <Database className="w-5 h-5 text-white" />
        </div>
        <div>
          <div className="text-base font-extrabold tracking-tight bg-gradient-to-r from-white via-slate-200 to-brand-400 bg-clip-text text-transparent">
            DataTrust<span className="text-brand-400">OS</span>
          </div>
          <div className="text-[10px] text-slate-400 font-mono">v1.0 • Enterprise Core</div>
        </div>
      </div>
      ...
      <div className="p-4 border-t border-slate-800/80 bg-slate-950/60 text-xs">
        ...
        <div className="mt-1 text-[10px] text-slate-500">100% Offline Capable • Zero External Keys</div>
      </div>
